...
let dollar_to_euro : dollar -> euro = fun (Dollar m) -> Euro (0.9249 *. m)
let euro_to_dollar : euro -> dollar = fun (Euro m) -> Dollar (1.0814 *. m)
...
let to_pound : currency -> currency =
 fun m ->
  match m with
  | Yen k -> Pound (k *. 0.0050)
  | Pound k -> m
  | SwedishCrown k -> Pound (k *. 0.0727)
...
let rec intbool_reverse ib_list =
   let rec pomozni_reverse ib_list acc =
      match ib_list with
      | Nil -> Nil
      | 
